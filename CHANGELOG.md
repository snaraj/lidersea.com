...
- `github/codeql-action/init` and `github/codeql-action/analyze` now move
  ...
  repository-wide workflow-integrity suite now structurally resolves quoted
  and unquoted references to either role and rejects a drifted SHA or version
  comment, with independent init-old and analyze-old mutants proving both
  directions fail closed. It covers both GitHub workflow extensions and
  case-insensitive action identity, including real-filesystem shadow-workflow,
  quoted-real, and skipped-decoy mutations.
